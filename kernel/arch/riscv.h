...
static inline auto r_mhartid() -> uint64_t {
  uint64_t x = 0;
...
static inline auto r_mie() -> uint64_t {
  uint64_t x = 0;
  asm volatile("csrr %0, mie" : "=r"(x));
  return x;
}
static inline auto w_mie(uint64_t x) -> void {
  asm volatile("csrw mie, %0" : : "r"(x));
}

static inline auto r_menvcfg() -> uint64_t {
  uint64_t x = 0;
  asm volatile("csrr %0, 0x30a" : "=r"(x));
  return x;
}
static inline auto w_menvcfg(uint64_t x) -> void {
  asm volatile("csrw 0x30a, %0" : : "r"(x));
}

static inline auto w_mcounteren(uint64_t x) -> void {
  asm volatile("csrw mcounteren, %0" : : "r"(x));
}
static inline auto r_mcounteren() -> uint64_t {
  uint64_t x = 0;
...
static inline auto w_mepc(uint64_t addr) -> void {
  asm volatile("csrw mepc, %0" : : "r"(addr));
};

static inline auto w_satp(uint64_t value) -> void {
  asm volatile("csrw satp, %0" : : "r"(value));
};

static inline auto r_satp() -> uint64_t {
  uint64_t x{};
  asm volatile("csrr %0, satp" : "=r"(x));
  return x;
}

static inline auto w_medeleg(uint64_t value) -> void {
  asm volatile("csrw medeleg, %0" : : "r"(value));
};

static inline auto w_mideleg(uint64_t value) -> void {
  asm volatile("csrw mideleg, %0" : : "r"(value));
};

static inline auto w_pmpaddr0(uint64_t value) -> void {
  asm volatile("csrw pmpaddr0, %0" : : "r"(value));
};
static inline auto w_pmpcfg0(uint64_t value) -> void {
  asm volatile("csrw pmpcfg0, %0" : : "r"(value));
};

static inline auto r_time() -> uint64_t {
  uint64_t x = 0;
...
static inline auto w_sie(uint64_t value) -> void {
  asm volatile("csrw sie, %0" : : "r"(value));
};
static inline auto r_sie() -> uint64_t {
  uint64_t rs{};
  asm volatile("csrr %0, sie" : "=r"(rs));
  return rs;
};

static inline auto sfence_vma() -> void {
  asm volatile("sfence.vma zero, zero");
};

static inline auto w_stvec(uint64_t value) -> void {
  asm volatile("csrw stvec, %0" : : "r"(value));
};

static inline auto w_sstatus(uint64_t value) -> void {
  asm volatile("csrw sstatus, %0" : : "r"(value));
};
static inline auto r_sstatus() -> uint64_t {
  uint64_t rs{};
  asm volatile("csrr %0, sstatus" : "=r"(rs));
  return rs;
};

static inline auto r_scause() -> uint64_t {
  uint64_t rs{};
  asm volatile("csrr %0, scause" : "=r"(rs));
  return rs;
};

static inline auto w_sepc(uint64_t x) -> void {
  asm volatile("csrw sepc, %0" : : "r"(x));
}
static inline auto r_sepc() -> uint64_t {
  uint64_t rs{};
  asm volatile("csrr %0, sepc" : "=r"(rs));
  return rs;
}

static inline auto r_stval() -> uint64_t {
  uint64_t rs{};
  asm volatile("csrr %0, stval" : "=r"(rs));
  return rs;
};

static inline auto r_stimecmp() -> uint64_t {
  uint64_t x = 0;
  asm volatile("csrr %0, 0x14d" : "=r"(x));
  return x;
}

static inline auto w_stimecmp(uint64_t x) -> void {
  asm volatile("csrw 0x14d, %0" : : "r"(x));
}

static inline auto w_tp(uint64_t x) -> void {
  asm volatile("mv tp, %0" : : "r"(x));
}
static inline auto r_tp() -> uint64_t {
  uint64_t x = 0;
...
static inline auto PG_ROUND_UP(uint64_t addr) -> uint64_t {
  return (addr + PGSIZE - 1) & ~(PGSIZE - 1);
};
static inline auto PG_ROUND_DOWN(uint64_t addr) -> uint64_t {
  return (addr) & ~(PGSIZE - 1);
};

static inline auto PTE2PA(uint64_t pte) -> uint64_t {
  return (pte >> 10U) << 12U;
};
static inline auto PA2PTE(uint64_t pa) -> uint64_t {
  return (pa >> 12U) << 10U;
};

static inline auto PTE_FLAGS(uint64_t pte) -> uint64_t {
  return pte & 1023U;  // (1023)_10 == (11,1111,1111)_2
};

constexpr uint32_t PX_MASK{0x1FF};
constexpr uint32_t PG_SHIFT{12};
static inline auto PX_SHIFT(uint32_t level) -> uint64_t {
  return PG_SHIFT + 9 * level;
};
static inline auto PX(uint32_t level, uint64_t pte) -> uint64_t {
...
static inline auto intr_on() -> void { w_sstatus(r_sstatus() | SSTATUS_SIE); }

static inline auto intr_off() -> void { w_sstatus(r_sstatus() & ~SSTATUS_SIE); }

static inline auto intr_get() -> uint64_t {
